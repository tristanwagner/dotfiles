...
docker run --name tranvpn -it --cap-add=NET_ADMIN \
  ...
  -e OPENVPN_CONFIG=my.protonvpn.udp \
  -e OPENVPN_USERNAME='' \
  -e OPENVPN_PASSWORD='' \
...
# OPENVPN_CONFIG https://github.com/haugene/vpn-configs-contrib/tree/main/openvpn/protonvpn
# OPENVPN_USERNAME  #https://account.protonvpn.com/account-password
